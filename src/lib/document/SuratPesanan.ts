...
export const generateSuratPesanan = async (data: DocumentData) => {
  ...
  doc.text('SURAT PESANAN', pageWidth - 14, headerY + 5, { align: 'right' });
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Dengan ini kami bermaksud memesan barang/jasa sebagai berikut:', 14, headerY + 70);
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('Total Pesanan:', pageWidth - 80, finalY);
  doc.text(formatCurrency(order.total_amount), pageWidth - 14, finalY, { align: 'right' });

  // Footer / Signatures
  const footerY = Math.max(finalY + 25, doc.internal.pageSize.getHeight() - 60);
